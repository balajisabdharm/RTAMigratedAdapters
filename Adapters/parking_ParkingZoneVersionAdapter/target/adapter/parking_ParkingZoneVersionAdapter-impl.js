...
function getParkingZoneVersion(dataObject){
 ...
 var getParkingZoneVersionRequest = '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:ws="http://ws.smartphone.infocomm.com/"><soapenv:Header/>'
          ...
          +'</soapenv:Body>'
          +'</soapenv:Envelope>';
 WL.Logger.warn("getParkingZoneVersion request | " + getParkingZoneVersionRequest)
 var input = {  
   method : 'post',
   headers :{
		"SOAPAction" : ""
  ...
  var result_getParkingZoneVersionResponse =  WL.Server.invokeHttp(input);
  ...
  if(result_getParkingZoneVersionResponse.Envelope.Body.getParkingZoneVersionResponse["return"].version == undefined || result_getParkingZoneVersionResponse.Envelope.Body.getParkingZoneVersionResponse["return"].version == null || result_getParkingZoneVersionResponse.Envelope.Body.getParkingZoneVersionResponse["return"].version == ''){
	var result_getParkingZoneVersionResponse_return =  WL.Server.invokeHttp(input);
 } else {
	 ...
	 WL.Logger.warn("result_getParkingZoneVersionResponse result | " + result_getParkingZoneVersionResponse_return + '}' +  JSON.stringify(result_getParkingZoneVersionResponse_return));
 }
  ...
  if(result_getParkingZoneVersionResponse.Envelope.Body.getParkingZoneVersionResponse["return"].version == undefined || result_getParkingZoneVersionResponse.Envelope.Body.getParkingZoneVersionResponse["return"].version == null || result_getParkingZoneVersionResponse.Envelope.Body.getParkingZoneVersionResponse["return"].version == ''){
	  ...
	  return {
		...
		,"statusCode":200
		 ...
		 WL.Logger.warn("getParkingZoneVersion result | " +  JSON.stringify(webServiceResult))
