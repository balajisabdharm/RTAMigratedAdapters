    ...
    try {
        ...
        if (authorizationCode) {
            ...
            var identity = {
                ...
                adapterLogger("submitAuthentication", "info", "Response Returned Successfully", identity);
...
function getUAEPASSProfile(authorizationCode, appId, appRedirectUri) {
    try {
        ...
        adapterLogger("getUAEPassProfile", "info", "Adapter Input", JSON.stringify([authorizationCode, appId, appRedirectUri]));
        ...
        //onLogout();//Amit Goyal IBM : moved to the base authentication class
        ...
        if (authorizationCode) {
            ...
            if (_UAEPassUserProfile != null && _RTAUserProfile != null) {
                ...
                var _serviceRelatedInfo = null;
                var finalResponse = null;
                adapterLogger("getUAEPassProfile", "info", "User Id : ", user_id);
                //var identity = {
                //    userId: user_id
                //}; //Amit Goyal IBM : moved to the base authentication class
                //WL.Server.setActiveUser("masterAuthRealm", identity); //Amit Goyal IBM : moved to the base authentication class
                ...
                while (trialsSetUserInfo > 0) {
                    ...
                    adapterLogger("getUAEPassProfile |setUserInfo", "info", "Request", JSON.stringify(invocationData));
                    var shellDatabaseResponse = MFP.Server.invokeProcedure(invocationData);
                    adapterLogger("getUAEPassProfile |setUserInfo", "info", "Response", JSON.stringify(shellDatabaseResponse));
                    // setUserIdentity(user_id);
                    if (shellDatabaseResponse && shellDatabaseResponse.isSuccessful) {
                        trialsSetUserInfo = 0;
                        ...
                        while (trialsGetUserProfile > 0) {
                            ...
                            if (fullUserProfileResponse && fullUserProfileResponse.isSuccessful) {
                                ...
                                adapterLogger("getUAEPassProfile", "info", "Response Returned Successfully", toString(finalResponse));
                                return finalResponse;
                            }
                            else {
                                trialsGetUserProfile--;
                ...
                var finalResponse = {
                status: 'Success',
                name: 'getUAEPassProfile',
                authRequired: false,
                UAEPassProfile: _UAEPassUserProfile,
                havePortalAccount: false
                    
                };
                adapterLogger("getUAEPassProfile", "info", "Response Returned Successfully with no profile", finalResponse);
                
                return finalResponse;
            } else {
                //return onAuthRequired(null, "Invalid code"); //Commented by IBM Amit Goyal
                var finalResponse = {
                status: 'Invalid code',
                name: 'getUAEPassProfile',
                };
        ...
        adapterLogger("getUAEPassProfile", "error", "Exception", toString(error));
        return handleError();
    }
}
...
//Amit Goyal IBM Unreachable Code...Method never used
